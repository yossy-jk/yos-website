...
const CATEGORIES = ["All", "Desks", "Seating", "Meeting", "Storage", "Breakout"];

const PIXELS_PER_CM = 0.5;

function ProductImage({ product, imageUrl }: { product: EOFProduct | ShopifyProduct; imageUrl?: string }) {
  ...
  // Fallback: category colour block
...
function SkeletonCard() {
  return (
    <div style={{
      ...
      <div style={{ width: 64, height: 64, borderRadius: 6, background: "#F0F0F0" }} />
      <div style={{ flex: 1 }}>
        <div style={{ height: 10, background: "#F0F0F0", borderRadius: 4, marginBottom: 6 }} />
        <div style={{ height: 8, width: "60%", background: "#F0F0F0", borderRadius: 4 }} />
      </div>
    </div>
  );
}
...
export default function ProductSidebar({ products: _propProducts }: ProductSidebarProps) {
  ...
  // Fetch Shopify products once on mount
  useEffect(() => {
    let cancelled = false;
    const fetchProducts = async () => {
      try {
        const res = await fetch('/api/space-planner-products');
        ...
        // Fall back to EOF products
        if (!cancelled) setShopifyProducts(null);
      } finally {
  ...
  // Build display list
  const displayProducts: Array<{ id: string; name: string; category: string; widthCm: number; depthCm: number; imageUrl?: string; eofProduct?: EOFProduct }> = (() => {
  ...
  const handleDragStart = (e: React.DragEvent<HTMLDivElement>, product: typeof filtered[0]) => {
    const data = {
      productId: product.id,
      name: product.name,
      category: product.category,
      price: 0,
      width: product.widthCm * PIXELS_PER_CM,
      depth: product.depthCm * PIXELS_PER_CM,
  ...
  return (
    <div style={{
      ...
      overflow: "hidden", background: "#FAFAFA", borderRight: "1px solid #E5E5E5",
    }}>
      {/* Header */}
        ...
        {!loading && filtered.map((product) => (
          <div
            ...
            style={{
              ...
              padding: "0.5rem 0.6rem", borderRadius: 8,
              background: "#FFFFFF", border: "1px solid #E5E5E5",
              cursor: "grab", userSelect: "none", transition: "box-shadow 0.1s",
            }}
            onMouseEnter={(e) => { (e.currentTarget as HTMLDivElement).style.boxShadow = "0 2px 6px rgba(0,0,0,0.08)"; }}
            onMouseLeave={(e) => { (e.currentTarget as HTMLDivElement).style.boxShadow = "none"; }}
          >
            <ProductImage
              product={{ id: product.id, name: product.name, category: product.category, widthCm: product.widthCm, depthCm: product.depthCm, type: product.category, tags: [], plannerProductId: product.id }}
