...
export default function Nav() {
  ...
  const NAV_H = 80 // px — matches h-20 on md+
  ...
  return (
    <>
      <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        ...
        >
          ...
          <div className="hidden md:flex gap-8 items-center">
            ...
            {/* Services dropdown — wrapper holds the ref for outside-click detection */}
            <div className="relative" ref={wrapperRef}>
              <button
                onClick={() => setServicesOpen(v => !v)}
                className="text-white/60 font-medium hover:text-white transition-colors duration-200 flex items-center gap-1.5 bg-transparent border-none cursor-pointer p-0"
                style={{ fontSize: '0.72rem', letterSpacing: '0.12em', textTransform: 'uppercase' }}
              >
                Services
                <svg
                  style={{ width: '0.6rem', height: '0.6rem', transition: 'transform 0.2s', transform: servicesOpen ? 'rotate(180deg)' : 'none', flexShrink: 0 }}
      ...
      {servicesOpen && (
        <>
          {/* Backdrop — shades the page, click to close */}
          <div
            className="fixed inset-0 z-40 hidden md:block"
            style={{ background: 'rgba(0,0,0,0.55)', top: `${NAV_H}px` }}
            onClick={() => setServicesOpen(false)}
          />

          {/* Panel — full-width, 5-column grid */}
          <div
            className="fixed left-0 right-0 bg-near-black border-b border-white/10 shadow-2xl z-50 hidden md:block"
            style={{ top: `${NAV_H}px` }}
          >
            <div
