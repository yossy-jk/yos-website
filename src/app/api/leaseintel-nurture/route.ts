...
async function patchContact(id: string, properties: Record<string, string>) {
  ...
  if (!res.ok) {
    const txt = await res.text()
    throw new Error(`HubSpot patch ${res.status}: ${txt.slice(0, 200)}`)
  }
  return res.json()
}
...
<div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; max-width: 600px; margin: 0 auto; padding: 24px; background: #ffffff;">
  ...
  <p style="color: #aaa; font-size: 11px; margin: 0;">Joseph Kelley — Your Office Space, Newcastle NSW<br><a href="https://yourofficespace.au" style="color: #00B5A5; text-decoration: none;">yourofficespace.au</a> · 0434 655 511</p>
</div>`
...
<div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; max-width: 600px; margin: 0 auto; padding: 24px; background: #ffffff;">
  ...
  <p style="color: #aaa; font-size: 11px; margin: 0;">Your Office Space, Newcastle NSW<br><a href="https://yourofficespace.au" style="color: #00B5A5; text-decoration: none;">yourofficespace.au</a> · 0434 655 511</p>
</div>`
...
export async function POST(req: Request) {
  try {
    ...
    let day2Results: {id: string; properties: Record<string,string>}[] = []
    try {
      const day2Search = await searchContacts({
        filterGroups: [
          {
            filters: [
              ...
              { propertyName: 'leaseintel_day2_sent', operator: 'NOT_HAS_PROPERTY' },
            ]
          },
          {
            filters: [
        ...
        properties: ['firstname', 'email', 'leaseintel_risk_level', 'leaseintel_day2_sent'],
        limit: 50,
      })
      day2Results = day2Search.results || []
    } catch (e) {
      console.error('Day 2 search error:', e)
    }

    // Deduplicate by contact id
    const seen = new Set<string>()
    for (const contact of day2Results) {
      if (seen.has(contact.id)) continue
      seen.add(contact.id)
      const { firstname, email, leaseintel_risk_level, leaseintel_day2_sent } = contact.properties
      if (!email || leaseintel_day2_sent === 'true') continue
      try {
    ...
    let day7Results: {id: string; properties: Record<string,string>}[] = []
    try {
      const day7Search = await searchContacts({
        filterGroups: [
          {
            filters: [
              ...
              { propertyName: 'leaseintel_day7_sent', operator: 'NOT_HAS_PROPERTY' },
            ]
          },
          {
            filters: [
        ...
        properties: ['firstname', 'email', 'leaseintel_risk_level', 'leaseintel_day7_sent'],
        limit: 50,
      })
      day7Results = day7Search.results || []
    } catch (e) {
      console.error('Day 7 search error:', e)
    }

    const seen7 = new Set<string>()
    for (const contact of day7Results) {
      if (seen7.has(contact.id)) continue
      seen7.add(contact.id)
      const { firstname, email, leaseintel_risk_level, leaseintel_day7_sent } = contact.properties
      if (!email || leaseintel_day7_sent === 'true') continue
      try {
        await resend.emails.send({
          from: 'Joseph Kelley <jk@yourofficespace.au>',
          to: email,
          ...
          html: DAY7_EMAIL(firstname || 'there', leaseintel_risk_level || 'medium'),
        })
    ...
    console.error('Nurture route error:', msg)
