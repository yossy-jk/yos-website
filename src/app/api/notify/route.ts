...
export async function POST(req: Request) {
  ...
  try {
    ...
    await resend.emails.send({
      ...
      to: TO,
      replyTo: email,
      subject: `New lead — ${name || email} via ${source}`,
      html: `<div style="font-family:-apple-system,sans-serif;max-width:600px;margin:0 auto;padding:24px">
        <div style="background:#0A0A0A;padding:20px 24px;margin-bottom:24px">
          <p style="color:${color};font-size:11px;letter-spacing:.3em;text-transform:uppercase;margin:0 0 4px">${source}</p>
          <p style="color:white;font-weight:700;font-size:18px;margin:0">New Lead${name ? ` — ${name}` : ''}</p>
        </div>
        <table style="width:100%;border-collapse:collapse;margin-bottom:24px">
          ${name ? `<tr><td style="padding:10px 0;border-bottom:1px solid #eee;color:#888;font-size:13px;width:120px">Name</td><td style="padding:10px 0;border-bottom:1px solid #eee;font-size:14px;font-weight:600">${name}</td></tr>` : ''}
          ...
          <tr><td style="padding:10px 0;border-bottom:1px solid #eee;color:#888;font-size:13px">Source</td><td style="padding:10px 0;border-bottom:1px solid #eee;font-size:14px">${source}</td></tr>
        </table>
        ${context ? `<div style="background:#f9f9f9;border-left:3px solid ${color};padding:16px 20px;margin-bottom:24px"><p style="color:#1A1A1A;font-size:14px;line-height:1.7;margin:0;white-space:pre-wrap">${context}</p></div>` : ''}
